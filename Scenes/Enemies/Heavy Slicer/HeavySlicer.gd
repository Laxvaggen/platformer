...
#Stay in place, attack player. If walks too far, return to starting location
...
func _roam_state(delta) -> void:
	...
	if can_see_target(pursue_target):
		_enter_pursue_state()
		return
	if global_position.x-spawn_location.x > 10:
		if global_position.x-spawn_location.x < 0:
			set_facing_x(1)
		else:
			set_facing_x(-1)
